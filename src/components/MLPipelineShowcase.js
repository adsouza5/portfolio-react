...
function SectionHeader({ children }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
      ...
      <span style={{ fontFamily:F.display, fontSize:15, letterSpacing:"3px", textTransform:"uppercase", color:C.text, fontWeight:700 }}>
        {children}
      </span>
    </div>
  );
...
function Tag({ children }) {
  return (
    <span style={{
      fontFamily:F.mono, fontSize:14, letterSpacing:"0.5px",
...
function MetricCard({ label, value, unit, color, note }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily:F.mono, fontSize:13, letterSpacing:"2px", textTransform:"uppercase", color:C.dim, marginBottom:10 }}>{label}</div>
      <div style={{ display:"flex", alignItems:"baseline", gap:5 }}>
        <span style={{ fontFamily:F.mono, fontSize:34, fontWeight:700, color:color||C.text, lineHeight:1 }}>{value}</span>
        {unit && <span style={{ fontFamily:F.mono, fontSize:14, color:C.dim }}>{unit}</span>}
      </div>
      {note && <div style={{ fontFamily:F.mono, fontSize:13, letterSpacing:"0.5px", color:C.dim, marginTop:6 }}>{note}</div>}
    </div>
  );
}
...
function LogEntry({ message, type, timestamp }) {
  ...
  return (
    <div style={{ fontFamily:F.mono, fontSize:15, lineHeight:1.85, display:"flex", gap:12 }}>
...
function LatencyChart({ history }) {
  ...
  return (
    <div>
      <div style={{ fontFamily:F.mono, fontSize:13, letterSpacing:"2px", textTransform:"uppercase", color:C.dim, marginBottom:10 }}>Latency (ms)</div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
        <span style={{ fontFamily:F.mono, fontSize:13, color:C.dim }}>0ms</span>
        <span style={{ fontFamily:F.mono, fontSize:13, color:C.dim }}>{max}ms</span>
      </div>
    </div>
  );
}
    ...
    <div style={{ background:C.bg, minHeight:"100vh", color:C.text, fontFamily:F.sans, overflowX:"hidden" }}>
      ...
      <button
        ...
        style={{
          ...
          fontFamily:F.mono, fontSize:14, letterSpacing:"2px", textTransform:"uppercase",
      ...
      <div style={{ maxWidth:1020, margin:"0 auto", padding:"68px 28px 60px", position:"relative", zIndex:1 }}>
        ...
        <div style={{ marginBottom:48, animation:"fadeUp 0.5s ease" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
            ...
            <span style={{ fontFamily:F.mono, fontSize:14, letterSpacing:"1.5px", color:C.dim }}>
          ...
          <p style={{ fontFamily:F.sans, fontSize:18, color:C.muted, lineHeight:1.85, maxWidth:600, marginBottom:20 }}>
        ...
        <div style={{ display:"flex", gap:0, marginBottom:32, animation:"fadeUp 0.5s ease 0.1s both" }}>
          {[{id:"architecture",label:"Architecture"},{id:"simulation",label:"Live Simulation"}].map(tab => (
            <button key={tab.id} onClick={()=>setView(tab.id)} style={{
              fontFamily:F.mono, fontSize:15, letterSpacing:"1.5px", textTransform:"uppercase",
        ...
        {view==="architecture" && (
          <div style={{ animation:"fadeUp 0.35s ease" }}>
              ...
              {ARCH.map(s => (
                <GlassCard key={s.title} style={{ padding:22, borderLeft:`3px solid ${s.color}`, boxShadow:`inset 0 0 40px ${s.color}08` }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
                    ...
                    <span style={{ fontFamily:F.display, fontSize:19, letterSpacing:"0.5px", color:C.text }}>{s.title}</span>
                  </div>
                  <p style={{ fontFamily:F.sans, fontSize:16, color:C.muted, lineHeight:1.8, marginBottom:14 }}>{s.desc}</p>
                  <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                    {s.details.map(d => (
                      <div key={d} style={{ fontFamily:F.mono, fontSize:15, color:C.dim, display:"flex", gap:8 }}>
            ...
            <GlassCard style={{ padding:"16px 22px", display:"flex", alignItems:"center", gap:16 }}>
              ...
              <div>
                <div style={{ fontFamily:F.mono, fontSize:16, letterSpacing:"1px", color:C.text, marginBottom:6 }}>Infrastructure as Code</div>
                <div style={{ fontFamily:F.sans, fontSize:16, color:C.muted, lineHeight:1.75 }}>
            ...
            <GlassCard style={{ padding:"24px 28px", marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24 }}>
                <div>
                  ...
                  <div style={{
                    fontFamily:F.mono, fontSize:16, letterSpacing:"1px",
                ...
                <button
                  ...
                  style={{
                    fontFamily:F.mono, fontSize:15, letterSpacing:"2px", textTransform:"uppercase",
                ...
                {pred ? (
                  <>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
                      ...
                      <span style={{
                        fontFamily:F.mono, fontSize:15, fontWeight:700, letterSpacing:"2px",
                    ...
                    <div style={{ display:"flex", gap:28, marginBottom:22 }}>
                      <div>
                        <div style={{ fontFamily:F.mono, fontSize:13, letterSpacing:"2px", textTransform:"uppercase", color:C.dim, marginBottom:6 }}>Confidence</div>
                        <div style={{ fontFamily:F.mono, fontSize:30, fontWeight:700, color:C.purple, textShadow:`0 0 16px ${C.purple}` }}>{(pred.confidence*100).toFixed(1)}%</div>
                      </div>
                      <div>
                        <div style={{ fontFamily:F.mono, fontSize:13, letterSpacing:"2px", textTransform:"uppercase", color:C.dim, marginBottom:6 }}>Latency</div>
                        <div style={{ fontFamily:F.mono, fontSize:30, fontWeight:700, color:pred.latency<25?C.green:C.amber, textShadow:`0 0 16px ${pred.latency<25?C.green:C.amber}` }}>{pred.latency}ms</div>
                      </div>
                    </div>
                    <LatencyChart history={latHist} />
                  </>
                ) : (
                  <div style={{ fontFamily:F.mono, fontSize:16, letterSpacing:"0.5px", color:C.dim, padding:"40px 0", textAlign:"center" }}>
              ...
              <GlassCard style={{ padding:22, display:"flex", flexDirection:"column" }}>
                ...
                <div ref={logRef} style={{
                  ...
                  <div style={{ position:"relative", zIndex:2 }}>
                    {logs.length===0 ? (
                      <div style={{ fontFamily:F.mono, fontSize:15, color:C.dim, padding:"40px 0", textAlign:"center" }}>
